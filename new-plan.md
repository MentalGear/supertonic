...
### Phase 2a: ceiling audit, then a subspace-ladder corpus — capacity measured
...
K=64 — and measure recovery as a function of K. That corpus is generated by
`py/phase2b_generate_subspace.py`; `py/phase2b_subspace_embed.py` and
`py/phase2b_subspace_probe.py` extract WavLM features and fit the ridge.

### Result (2026-09-09): audio carries the perturbation, uniformly across components; no capacity ceiling visible through K=64

The subspace ladder is sized so the estimator can express its target
(`n_train` at or above `K`, unlike 2b's `n_train=240` against `d=6,120`), so
this measures what 2b's design could not. Fixed base preset M1, eps=0.20,
WavLM layers 3-5 mean+std, ridge (`fit_ridge` reused from `phase2b_probe`):

| K | n_train | oracle ceiling | achieved mean R² | per-component range | shuffled | loudness | cosine |
|---|---|---|---|---|---|---|---|
| 4 | 240 | 1.0000 | 0.912 | 0.884–0.926 | −0.002 | 0.023 | 0.968 |
| 16 | 240 | 1.0000 | 0.608 | 0.499–0.701 | −0.016 | −0.004 | 0.798 |
| 64 | 480 | 1.0000 | 0.232 | −0.002–0.351 | −0.008 | −0.003 | 0.489 |

Same engine, same WavLM features, same estimator family as Phase 2b; the only
change is asking for a target the estimator has the rank to express. The
oracle ceiling of 1.0000 at every K is checked, not assumed — it is exactly
the diagnostic 2b lacked (see the correction above) — and confirms the
design has the power to detect recovery if the audio carries it. It does:
recovery is uniform across components (the per-component range at each K
does not fall to zero anywhere) rather than carried by a few dimensions,
the shuffled-target control sits at noise level at every K, and the
loudness-only control stays near zero, so the recovered signal is not a
"perturbation buys loudness" leak. **Established: `style_ttl` perturbations
in a K-dimensional subspace are recoverable from audio, well above chance,
up to at least K=64.** Not established: a capacity limit — R² falling with
K here is confounded with two other things that change alongside it, both
addressed below.

**Amplitude-matched control, to separate "more dimensions" from "less signal
per dimension."** The ladder held total perturbation energy fixed while
spreading it over K directions, so per-coefficient amplitude falls as
1/√K — a K=64 coefficient is perturbed at a quarter the amplitude of a K=4
one at the same eps. Regenerating K=4 at eps=0.05 and K=16 at eps=0.10
matches per-direction amplitude to K=64 at eps=0.20:

| K | matched eps | mean R² | fixed-energy R² (from table above) |
|---|---|---|---|
| 4 | 0.05 | 0.297 | 0.912 |
| 16 | 0.10 | 0.199 | 0.608 |
| 64 | 0.20 | 0.232 | 0.232 |

Amplitude-matched, the curve is nearly flat (0.297 / 0.199 / 0.232) against
the fixed-energy curve's steep fall (0.912 / 0.608 / 0.232). So most of the
fixed-energy decline with K was per-direction signal-to-noise, not a limit
on how many dimensions the audio carries — **no capacity ceiling is visible
up to K=64.** Caveat: the K=4→K=16 step still falls (0.297→0.199) under
amplitude matching, but `n_train` is pinned at 240 for both while the target
quadruples, so the estimator is fitting four times the coefficients from the
same data — a rank effect this data cannot separate from a genuine capacity
difference at that step.

**Scaling sweep (`py/phase2a_scaling.py`): does recovery saturate with more
training data, at fixed K?** Test split held fixed across every `n_train`
and every K; 5 random subsamples of the training split per point. Mean
per-component R² by `n_train`:

| K | 30 | 60 | 120 | 240 | 360 | 480 |
|---|---|---|---|---|---|---|
| 4 | 0.42 | 0.70 | 0.85 | 0.91 | — | — |
| 16 | −0.03 | 0.18 | 0.41 | 0.61 | — | — |
| 64 | −0.04 | −0.02 | 0.00 | 0.12 | 0.18 | 0.23 |

The oracle ceiling (computed per point, same as above) binds only at K=64
with `n_train` 30 (ceiling 0.46) and 60 (ceiling 0.94) — those two points
measure estimator rank, not audio. From `n_train=120` on, the ceiling is
1.00 everywhere, so every other point in the table measures audio
recoverability. K=4 is visibly flattening: the last doubling of `n_train`
(120→240) moves R² by 0.088, against an overall slope of 0.234 per log
`n_train` across the full sweep. K=64 shows no flattening at all: overall
slope 0.163 per log `n_train`, and the *last* doubling (240→480) moves it by
0.177 — as steep as the average, not less.

**Sizing the next corpus, stated with its limits.** Extrapolating K=64's
fit to R² ≈ 0.5 suggests `n_train` ≈ 2,500, but that is a 5x extrapolation
from four points on a curve that must eventually bend (R² is bounded above,
a log-linear fit cannot hold indefinitely). The honest statement is: the
data supports "still rising steeply, no saturation through `n_train=480`"
and does not support a confident number beyond roughly `n_train=1,000`.

**The prize this sets up.** At `n_train` around 6,144 the subspace
restriction can be dropped entirely and Phase 2b's original question — can
audio predict the full active-row `style_ttl` tensor, not a K-dimensional
slice of it — becomes well-posed for the first time, since `n_train` would
finally exceed the target's ambient dimension. That needs roughly 8,000
renders, about 3 hours of CPU on this 4-core box to generate — which is why
it moves to a GPU session for the fit itself
(`docs/phase2a_scaling_colab.ipynb`, being written now).

**Refinement is buildable, independent of the encoder question above.** All
four ONNX models convert under `onnx2torch` with two generic version patches
(`py/onnx2torch_patches.py`); gradients reach `style_ttl` on 12,800 of
12,800 elements; and the converted `text_encoder` matches `onnxruntime` to
1.8e-6. The binding cost is speed, not correctness: about 123 s per gradient
step on this CPU at a toy sequence length, since the converted graph is
unfused and runs 13-19x slower than `onnxruntime`. One shortcut is rejected
on the record: optimizing against `text_emb` alone would be roughly 30x
cheaper per step but constrains nothing in `vector_estimator`, which reads
